 ...
 * computing an overlap. `activePieceId` alone means a standalone draft.
 * Inside an idea, the idea's own Write/Pieces choice decides: Pieces has no
 * piece open in the editor, so the comment goes to the idea; Write attaches it to
...
): { pieceId: string | null; ideaId: string | null } | null {
  if (activeIdeaId && ideaSpace === "pieces") return { pieceId: null, ideaId: activeIdeaId };
  if (activePieceId) return { pieceId: activePieceId, ideaId: null };
  if (activeIdeaId) return { pieceId: null, ideaId: activeIdeaId };
  return null;
}
...
export function visibleComments(
  ...
  home: { pieceId: string | null; ideaId: string | null } | null,
): Comment[] {
  if (!home) return [];
  const all = Array.isArray(comments) ? comments : Object.values(comments);
  return (all as Comment[])
    .filter((c) => c.pieceId === home.pieceId && c.ideaId === home.ideaId)
