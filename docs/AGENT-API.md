...
- **Browser / self-hosted server:** a browser can't read your local filesystem, so the app polls its own Next.js server every 10 seconds:
  - `GET /api/v1/agent-inbox?since=<epoch-ms>` — lists pending `.md` handoff files that a `since` cursor hasn't seen yet, plus every idea's `resources.jsonl` AND `idea.json` manifest, both always in full (their ingestion is idempotent by id, and manifests are never moved — fragment-mcp still reads them for `list_ideas`/`get_piece`). Manifests are imported before piece files each poll, so a piece referencing an agent-created `idea_id` resolves even in its first batch. Recurses through per-idea subdirectories; never follows a client-supplied path.
