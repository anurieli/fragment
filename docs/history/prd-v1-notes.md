> **Superseded.** This is Fragment's first product requirements document, kept
> for the record. It describes the notes-era product: a three-panel writing app
> built around a `Note` as the primary entity, with a sidebar of standalone
> notes, an editor, and a Snip Bar, plus the Content Engine (ideas and pieces)
> bolted on beside it as a second space. It has no accounts, no sync, and no
